...
class TomographyEM(dinv.physics.LinearPhysics):
    ...
    backend) to project a reconstructed volume down to its tilt series.
       ...
       :param tuple[int,int,int] volume_shape: Native ``(D, H, W)`` shape of the
        input volume, in whatever axis order the caller uses.
    :param angles_deg: 1D array/tensor of tilt angles in degrees (e.g. read from
        a ``.tlt`` file).
    :param tuple[int,int,int] axis_order: Permutation of ``(0, 1, 2)`` mapping
        native volume axes to astra's ``(n_slices, n_rows, n_cols)`` convention,
        where ``n_slices`` is the rotation-invariant (tilt) axis and
        ``(n_rows, n_cols)`` form the rotating 2D plane. Default ``(0, 1, 2)``
        (no permutation).
    :param tuple[int,int] | None detector_shape: Real detector pixel grid
        ``(V, N)``. Decoupled from ``volume_shape``/``axis_order`` — it is a
        property of the camera, not of the reconstruction geometry. Defaults to
    ...
    def __init__(
        ...
        self._axis_order = tuple(int(a) for a in axis_order)
        self._inv_axis_order = [0, 0, 0]
        for j, a in enumerate(self._axis_order):
            self._inv_axis_order[a] = j

        astra_img_size = tuple(self.volume_shape[a] for a in self._axis_order)

        if detector_shape is None:
            detector_shape = (self.volume_shape[0], self.volume_shape[1])
    ...
    # ------------------------------------------------------------------
    # Axis-order bookkeeping between this repo's native (D, H, W) volumes
    # and astra's (n_slices, n_rows, n_cols) convention.
    # ------------------------------------------------------------------

    def _to_astra(self, x: torch.Tensor) -> torch.Tensor:
        """(B, C, *native D,H,W) -> (B, C, *astra n_slices,n_rows,n_cols)."""
        perm = (0, 1) + tuple(2 + a for a in self._axis_order)
        return x.permute(*perm).contiguous()

    def _from_astra(self, x: torch.Tensor) -> torch.Tensor:
        """(B, C, *astra n_slices,n_rows,n_cols) -> (B, C, *native D,H,W)."""
        perm = (0, 1) + tuple(2 + a for a in self._inv_axis_order)
        return x.permute(*perm).contiguous()
    ...
    def A(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        """Forward projection: native volume (B,C,D,H,W) -> sinogram (B,C,V,A,N).
        ...
        """
        return self.xray.A(self._to_astra(x))

    def A_adjoint(self, y: torch.Tensor, **kwargs) -> torch.Tensor:
        """Approximate adjoint (pixel-driven back-projection): sinogram -> native volume.

        :param torch.Tensor y: Sinogram of shape (B, C, V, A, N).
        :return: Volume of shape (B, C, *volume_shape).
        """
        return self._from_astra(self.xray.A_adjoint(y))
